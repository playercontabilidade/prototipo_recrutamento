# Graph Report - Prototipação Vagas  (2026-09-17)

## Corpus Check
- 73 files · ~190,284 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1980 nodes · 3900 edges · 141 communities (125 shown, 16 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 67 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `dfad619c`
...
- Range inventory
- placeCandidateMoreMenu
- Task 1
- confirmPipelineAction
...
1. `escapeHtml()` - 137 edges
2. `showToast()` - 91 edges
3. `normalize()` - 74 edges
4. `renderCandidateDetails()` - 32 edges
5. `goToPage()` - 31 edges
6. `setCandidatePortalView()` - 30 edges
7. `renderGestorPortal()` - 29 edges
8. `formatBRDate()` - 28 edges
9. `renderPipeline()` - 27 edges
10. `runPipelineCandidateAction()` - 27 edges

## Surprising Connections (you probably didn't know these)
- `renderTalents()` --indirect_call--> `talentTemplate()`  [INFERRED]
  app.js → app.js  _Bridges community 106 → community 52_
- `renderResults()` --indirect_call--> `resultTemplate()`  [INFERRED]
  app.js → app.js  _Bridges community 106 → community 95_
- `renderAgenda()` --indirect_call--> `interviewTemplate()`  [INFERRED]
  app.js → app.js  _Bridges community 106 → community 6_
- `renderPipeline()` --indirect_call--> `candidateCardTemplate()`  [INFERRED]
  app.js → app.js  _Bridges community 124 → community 77_
...
Cohesion: 0.08
Nodes (40): analysisStatusMeta(), blankHiringRequest(), canApproveHiringRequest(), canGestorApproveRequests(), commitGestorRequestDecision(), confirmBookingSlot(), ensureHiringRequestReadyForRhDecision(), fillGestorRequestForm() (+32 more)
...
Cohesion: 0.17
Nodes (16): dossierTabForPendingType(), focusPendingTarget(), openCandidate(), openOfferDialog(), openPendingInterviewResult(), openPendingInterviewSheet(), openPendingJobApproval(), openPendingOffer() (+8 more)

### Community 6 - "dayKey"
Cohesion: 0.13
Nodes (26): calendarEventsForMonth(), countGestorInterviewBuckets(), dayKey(), fillAgendaVacancyFilter(), formatInterviewGroupLabel(), fullCalendarEventClass(), gestorInterviewBucket(), getAgendaInterviews() (+18 more)

### Community 7 - "getFitQuestionById"
Cohesion: 0.27
Nodes (10): addFitPillar(), duplicateFitQuestion(), getFitQuestionById(), openFitQuestionEditor(), runFitHubPrimaryAction(), saveFitModel(), saveFitQuestion(), setFitHubTab() (+2 more)

### Community 8 - "renderCandidateHome"
Cohesion: 0.24
Nodes (23): buildCandidateNotifications(), candidateVisibleStage(), ensureInterviewConfirmDeadline(), expireUnconfirmedCandidateInterviews(), formatCandidateInterviewDate(), getCandidateFitItems(), getCandidateHomeHistoryItems(), getCandidateInterviewItems() (+15 more)
...
### Community 12 - "openInterviewEditor"
Cohesion: 0.31
Nodes (9): applyInterviewCandidateSelection(), defaultInterviewTimeEnd(), fillInterviewInterviewers(), fillInterviewSheetOptions(), fillInterviewStageOptions(), getActiveSheetTemplates(), interviewInterviewerOptions(), openInterviewEditor() (+1 more)
...
Cohesion: 0.18
Nodes (11): 04 — Modais, A) `stack-dialog` — fluxos e pickers, B) `settings-form-dialog` — catálogos e formulários RH, C) `interview-detail-dialog` — detalhe entrevista, Cancelar ≠ Fechar, Comportamento JS, D) `candidate-filter-dialog` — filtros candidato, Footer de detalhe + menu ⋮ (+3 more)
...
### Community 30 - "submitInterview"
Cohesion: 0.13
Nodes (27): clearConductAutosaveTimer(), closeInterviewConductReview(), confirmCandidateInterview(), confirmInterviewRequest(), ensureInterviewMeetLink(), finalizeConductInterview(), findInterviewConflicts(), formatInterviewWhen() (+19 more)
...
### Community 33 - "setCandidatePortalView"
Cohesion: 0.12
Nodes (29): candidateJobAvailability(), candidateJobDatePanel(), candidateMetaGrid(), closeTopbarPopovers(), formatPublicDate(), getActiveCandidateApplyResumeName(), getCandidateApplyJobs(), getCandidateBulkApplyJobs() (+21 more)
...
Cohesion: 0.15
Nodes (22): closeFitMoreActions(), computeFitAdherence(), createFitAssignment(), defaultFitModelForJob(), fitAssignmentActionsFor(), fitModelTitle(), fitOptionScore(), fitQuestionMaxScore() (+14 more)

### Community 46 - "openScoreEvaluationDetail"
Cohesion: 0.13
Nodes (34): activeScorecardForVacancy(), autoScoreForCriterion(), computeOverallScore(), defaultScorecardCriteria(), deriveEvaluationStatus(), duplicateScorecard(), emptyScoreRows(), ensureScoreEvaluation() (+26 more)
...
Cohesion: 0.14
Nodes (20): applyTalentEdit(), approveCandidateToTalentBank(), blockCandidate(), clearTalentAdvancedFilters(), closeTalentDrawer(), closeTalentEditDialog(), getFilteredTalents(), inviteTalentToJob() (+12 more)

### Community 53 - "openCandidateTestDetail"
Cohesion: 0.20
Nodes (21): assignmentActionsFor(), assignmentCatalog(), assignmentStatusClass(), assignmentTitle(), downloadAssignmentPdf(), formatAssignmentDate(), getAssignmentById(), getFilteredAssignments() (+13 more)
...
### Community 58 - "normalize"
Cohesion: 0.20
Nodes (19): candidateApplicationMatchesFilter(), candidateApplicationTimeline(), candidateAppsFilterLabel(), candidateAppStageClass(), computeMatch(), findCandidateByEmail(), getCandidateAppNextHint(), getCandidateHomeMatchAverage() (+11 more)
...
### Community 63 - "Design: Workspace contínuo (layout RH)"
Cohesion: 0.08
Nodes (24): 1. Chrome e hierarquia, 2. Superfície de conteúdo, 3. Linguagem visual, 4. Escopo de implementação, 5. Arquivos prováveis, 6. Relação com specs anteriores, Anti-redundância, Aprovação (+16 more)
...
Cohesion: 0.18
Nodes (17): applyGestorCandidateDecision(), closeGestorCandidateDecisionDialog(), commitGestorCandidateDecision(), findAnalysisForCandidate(), findCandidateForAnalysis(), openCandidateResume(), openGestorAnalysis(), openGestorCandidatePanel() (+9 more)
...
### Community 72 - "proposalSalary"
Cohesion: 0.17
Nodes (24): applyCounterFieldsToOffer(), applyOfferFormValues(), archiveProposalVersion(), buildCounterProposalFromValues(), counterProposalPanelMarkup(), createEmptyProposal(), ensureCandidateProposal(), ensureProposalShape() (+16 more)

### Community 73 - "pushTimelineEvent"
Cohesion: 0.17
Nodes (15): addCandidateComment(), assignmentsForCandidate(), buildCandidateTimeline(), ensureCandidateComments(), ensureCandidateGestorNotes(), ensureCandidateTimeline(), formatTimelineAt(), getCandidateTestScore() (+7 more)

### Community 74 - "openTestPreview"
Cohesion: 0.11
...
Cohesion: 0.22
Nodes (13): applyPipelineFiltersFromDialog(), clearPipelineAdvancedFilters(), clearPipelineSelection(), clearSinglePipelineFilter(), closePipelineFiltersDialog(), getActivePipelineFilters(), openPipelineFiltersDialog(), renderPipeline() (+5 more)

### Community 78 - "runPipelineCandidateAction"
Cohesion: 0.22
Nodes (11): contactPreviewText(), moveCandidate(), openContactDialog(), openManagerDialog(), openMoveStageDialog(), openOrCreateScoreEvaluation(), openPendingContact(), runPipelineCandidateAction() (+3 more)

### Community 79 - "Task 5 Review — Density polish + hub motion + cache + graphify"
Cohesion: 0.10
Nodes (19): Critical, Findings, Important, Issues, Mandatory verification, Minor, Overlap / redundancy (anti-redundância), Recommendation (+11 more)

### Community 80 - "renderCandidateOffer"
Cohesion: 0.18
Nodes (12): candidateAppNextSteps(), candidatePortalHasActiveOffer(), candidatePortalHasActivePreAdmission(), candidateVacancyPickerCard(), getPortalActiveProposal(), getPortalPipelineCandidate(), getPortalPreAdmission(), markPortalOfferViewed() (+4 more)

### Community 81 - "renderPreAdmissions"
Cohesion: 0.10
Nodes (26): addPreAdmissionPendency(), closePreAdmissionDialog(), defaultPreAdmissionChecklist(), documentStatusClass(), documentTypeMark(), ensureCandidateDocuments(), finalizePreAdmission(), findPreAdmissionForCandidate() (+18 more)
...
### Community 84 - "Spec compliance"
Cohesion: 0.11
Nodes (18): Critical, Design spec alignment (`2026-09-15-workspace-continuo-layout-design.md`), Findings, Global constraints checklist, Important, Issues, Minor, Overlap / redundancy (anti-redundância) (+10 more)
...
Cohesion: 0.22
Nodes (14): applyPipelineBulkStage(), confirmMoveStage(), ensureJobInstrumentsForCandidate(), fitAssignmentsForCandidate(), isInterviewPipelineStage(), isTestPipelineStage(), jobInstrumentIds(), maybeOfferInterviewSchedule() (+6 more)
...
### Community 95 - "goToPage"
Cohesion: 0.21
Nodes (13): closeOverlayDialogs(), dismissCandidate(), getFilteredResults(), goToPage(), handleGoTarget(), hideCandidate(), openDismissDialog(), removeCandidateFromPipeline() (+5 more)

### Community 96 - "openJobDetails"
Cohesion: 0.13
Nodes (29): candidateAvatars(), closeJobMoreActions(), convertPreAdmissionToEmployee(), duplicateJob(), ensureJobDefaults(), getFilteredJobs(), jobActionsForStatus(), jobCanHire() (+21 more)
...
### Community 99 - "showPage"
Cohesion: 0.20
Nodes (12): closeGestorJobView(), closeSidebar(), gestorHashFromState(), gestorNavKeyFromState(), hubHash(), renderSelecaoHub(), showPage(), syncGestorSidebarChrome() (+4 more)
...
### Community 102 - "renderPendenciasPage"
Cohesion: 0.07
Nodes (43): buildGestorPendencies(), buildPendencies(), collectGestorOverviewMetrics(), countGestorPendencies(), countPendenciesByBucket(), findPendingItemById(), formatGestorPendDueLabel(), formatPendingDue() (+35 more)
...
Cohesion: 0.13
Nodes (25): agendaEventChip(), escapeHtml(), fillCandidateProfileForm(), fillTalentFilterOptions(), formatMoney(), formatShortDate(), formatTalentMoney(), initials() (+17 more)

### Community 107 - "getStrategicAnalytics"
Cohesion: 0.17
Nodes (12): candidateToDrillItem(), countBy(), daysInStage(), exportAnalyticsStub(), getAnalyticsUniverse(), getStrategicAnalytics(), inAnalyticsPeriod(), jobToDrillItem() (+4 more)
...
### Community 110 - "Task 1 Review — Shared chip styles + retire segmented chrome"
Cohesion: 0.11
Nodes (17): Critical, Findings, Global constraints checklist, Important, Issues, Minor, Overlap / redundancy (anti-redundância), Recommendation (+9 more)

### Community 111 - "openInterviewDetail"
Cohesion: 0.27
Nodes (10): closeCandidateMoreActions(), closeGestorAnalysisMoreMenu(), closeInterviewMoreActions(), forceCloseAllDialogs(), openFromShareHash(), openInterviewDetail(), openTimelineRelated(), parseShareEntityHash() (+2 more)

### Community 112 - "renderGestorJobView"
Cohesion: 0.24
Nodes (10): gestorFunnelCounts(), gestorFunnelStageId(), gestorJobCandidates(), jobDaysOpen(), jobSlaLabel(), jobSlaLabelPt(), renderGestorJobCandidatesTable(), renderGestorJobFunnel() (+2 more)

### Community 113 - "Task 1 Report — Shared chip styles + retire segmented chrome"
Cohesion: 0.12
Nodes (16): Commands, Commits, Commits, Concerns, Fix round 1/5 (review `task-1-review.md`), How verified, Hub L2 (underline), Self-review (+8 more)

### Community 114 - "Range inventory"
Cohesion: 0.14
Nodes (13): Anti-redundância, Assessment, Critical (Must Fix), Deferred minors — triage, Final review — Workspace Contínuo layout wave, Important (Should Fix), Issues, Minor (Nice to Have) (+5 more)

### Community 115 - "placeCandidateMoreMenu"
Cohesion: 0.25
Nodes (8): closeAssignmentMoreActions(), placeCandidateMoreMenu(), resolveMoreMenuPlacement(), toggleAssignmentMoreActions(), toggleCandidateMoreActions(), toggleGestorAnalysisMoreMenu(), toggleInterviewMoreActions(), toggleJobMoreActions()

### Community 116 - "Task 1"
Cohesion: 0.14
Nodes (13): Fix round 1/5 open, Preflight scan, Ruling: Important #1 (scope creep sidebar), Ruling: work in place on main, SDD ledger — plan: docs/superpowers/plans/2026-09-15-workspace-continuo-layout.md, Setup, Task 1, Task 1: complete (commits 7595acb..f08b965, review clean) (+5 more)

### Community 117 - "confirmPipelineAction"
Cohesion: 0.40
Nodes (5): applyCatalogTestToCandidate(), confirmPipelineAction(), findReusableFitAssignment(), openFitApplyDialog(), registerCandidateOperationalEvent()

### Community 118 - "Task 5 Report — Density polish + hub motion + cache + graphify"
Cohesion: 0.15
Nodes (12): Cache (`index.html`), Commits, Concerns, Density (`styles.css`), graphify, Motion (`styles.css`), QA checklist, Status (+4 more)

### Community 119 - "Task 1 Re-review — Fix round 1"
Cohesion: 0.18
Nodes (10): 1. Important — Scope creep sidebar (`.main-nav`, `.content-scroll`, etc.), 2. Important — Restore `.hub-chrome` padding `0 20px`, 3. Minor — Mirror `.is-active` + active count/icon on gestor bucket IDs / wrappers, Executive summary, Finding verdicts (verification set), New breakage (fix diff `11189e9..f08b965` only), Recommendation, Return block (controller) (+2 more)

### Community 120 - "Task 2 Report — Pendências RH buckets as toolbar chips"
Cohesion: 0.18
Nodes (10): Commits, Concerns, Overlap checklist (anti-redundância), Self-review, Status, Task 2 Report — Pendências RH buckets as toolbar chips, Test summary, Verification (+2 more)

### Community 121 - "Task 3 Report — Talentos listas as chips in talent toolbar"
Cohesion: 0.18
Nodes (10): Commits, Concerns, Overlap checklist (anti-redundância), Self-review, Status, Task 3 Report — Talentos listas as chips in talent toolbar, Test summary, Verification (+2 more)

### Community 122 - "Task 4 Report — Gestor buckets → workspace-filter-chips"
Cohesion: 0.18
Nodes (10): Commits, Concerns, Overlap checklist (anti-redundância), Self-review, Status, Task 4 Report — Gestor buckets → workspace-filter-chips, Test summary, Verification (+2 more)

### Community 123 - "File map"
Cohesion: 0.20
...
### Community 124 - "candidateCardTemplate"
Cohesion: 0.60
Nodes (5): candidateCardTemplate(), candidateComments(), cardPanelMarkup(), spriteIcon(), stageClass()

### Community 125 - "renderGestorCandidatePanel"
Cohesion: 0.19
Nodes (15): candidateHasPendingGestorAnalysis(), emptyProcessValue(), gestorAnalysisToolbarIcon(), gestorCandidateDaysInProcess(), gestorCandidateResponsible(), gestorCandidateRow(), gestorCandidateTags(), gestorFitSummary() (+7 more)

### Community 126 - "renderSettings"
Cohesion: 0.13
Nodes (16): fillNewJobFromHiringRequest(), getCatalogTestsByType(), getHiringApprovalFlow(), openApplyTestForCandidate(), openNewJobFromHiringRequest(), openNewJobPage(), openPipelineActionDialog(), renderNewJobCatalogs() (+8 more)

### Community 128 - "renderGestorPortal"
Cohesion: 0.18
Nodes (19): applyHiringRequestDecision(), closeGestorRequestForm(), closeHiringRequestDecisionDialog(), myGestorJobs(), openGestorInterviewRequestFollowup(), openGestorJob(), openGestorRequestForm(), proposeInterviewRequestSlot() (+11 more)

### Community 129 - "openFitModelEditor"
Cohesion: 0.50
Nodes (5): fitQuestionTypeLabel(), getFitPillarById(), openFitModelEditor(), renderFitBanco(), saveFitPillarCard()

### Community 130 - "getCandidateProfileCompletion"
Cohesion: 0.50
Nodes (4): candidateActionMapMarkup(), candidateNextActionsMarkup(), getCandidateProfileCompletion(), getCandidateProfileCompletionData()

### Community 131 - "renderTalentFilterChips"
Cohesion: 1.00
Nodes (3): getActiveTalentFilters(), renderTalentFilterChips(), syncTalentFilterButton()

### Community 140 - "submitGestorInterviewRequest"
Cohesion: 0.33
...
## Knowledge Gaps
- **893 isolated node(s):** `jobs`, `candidates`, `candidateSkillDefaults`, `candidateStageEnteredDefaults`, `candidateConsentDefaults` (+888 more)
...
- **Why does `escapeHtml()` connect `escapeHtml` to `app.js`, `openFitModelEditor`, `getCandidateProfileCompletion`, `formatBRDate`, `renderGestorPortal`, `renderTalentFilterChips`, `dayKey`, `getFitQuestionById`, `renderCandidateHome`, `renderCandidateHeaderTags`, `fillInterviewCandidateOptions`, `openGestorCandidateDecisionDialog`, `openInterviewEditor`, `submitInterview`, `setCandidatePortalView`, `renderAnalytics`, `openConductHistory`, `openFitAssignmentDetail`, `openScoreEvaluationDetail`, `renderTalents`, `openCandidateTestDetail`, `normalize`, `getSheetTemplateById`, `showToast`, `proposalSalary`, `pushTimelineEvent`, `openTestPreview`, `renderPipeline`, `runPipelineCandidateAction`, `renderCandidateOffer`, `renderPreAdmissions`, `renderCandidateDetails`, `goToPage`, `openJobDetails`, `renderPendenciasPage`, `sheetTemplateCard`, `renderGestorJobView`, `confirmPipelineAction`, `candidateCardTemplate`, `renderGestorCandidatePanel`, `renderSettings`?**
  _High betweenness centrality (0.002) - this node is a cross-community bridge._
- **Why does `showToast()` connect `showToast` to `app.js`, `renderGestorPortal`, `formatBRDate`, `openFitModelEditor`, `runPendenciaAction`, `getFitQuestionById`, `submitGestorInterviewRequest`, `submitInterview`, `setCandidatePortalView`, `openFitAssignmentDetail`, `openScoreEvaluationDetail`, `renderTalents`, `openCandidateTestDetail`, `getSheetTemplateById`, `proposalSalary`, `openTestPreview`, `runPipelineCandidateAction`, `renderCandidateOffer`, `renderPreAdmissions`, `renderCandidateDetails`, `saveGestorRequestDraft`, `goToPage`, `openJobDetails`, `renderPendenciasPage`, `getStrategicAnalytics`, `openInterviewDetail`, `confirmPipelineAction`, `renderSettings`?**
...
- **Should `formatBRDate` be split into smaller, more focused modules?**
